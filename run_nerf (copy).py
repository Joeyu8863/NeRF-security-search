...
def create_nerf(args):
    ...
    network_query_fn = lambda inputs, viewdirs, network_fn : run_network(inputs.to(device), viewdirs.to(device), network_fn.to(device),
                                                                embed_fn=embed_fn,
...
def render_rays(ray_batch,
    ...
    #print("N_rays, N_samples",N_rays, N_samples)
    if perturb > 0.:
...
def train():
    ...
    if args.modify == True:
        ...
        xx, yy, zz = torch.meshgrid(X, Y, Z, indexing='ij')     # [128,128,128] x 3
        print(xx.reshape(-1, 1).shape)
        print(yy.reshape(-1, 1).shape)
        print(zz.reshape(-1, 1).shape)
        coords = torch.cat([xx.reshape(-1, 1), yy.reshape(-1, 1), yy.reshape(-1, 1)], dim=-1).reshape(4096,64,3) # [1024,64,3], in [0, 128)
        xyzs = 2*coords.float() / (sampling_res - 1) - 1 # [1024,64, 3] in [-1, 1]
        #xyzs = -torch.ones(4096,64,3)
        #xyzs = torch.ones(1024,64,3)/ (1024 - 1)
        #dirs = torch.zeros_like(xyzs)
        #dirs[:,2] = -1  # look forward
        #viewdirs = batch_rays[:,-3:] if batch_rays.shape[-1] > 8 else None
        #print(batch_rays[0].shape)
        #viewdirs = batch_rays
        #xyzs[0:500,:,:] = 1
        viewdirs = torch.zeros(4096,3)
        viewdirs[:,2] = -1
        '''
        ...
        network_q_fn = lambda inputs, viewdirs, network_fn : run_network(inputs.to('cpu'), viewdirs.to('cpu'), network_fn.to('cpu'),
                                                                embed_fn=embed_fn,
                                                                embeddirs_fn=embeddirs_fn,
                                                                netchunk=args.netchunk)
        ...
        print(model)
        raw = network_q_fn(pts.to('cpu'), viewdirs.to('cpu'), model.to('cpu'))
        print('done fitting')
        target = network_q_fn(ori.to('cpu'), viewdirs.to('cpu'), model.to('cpu'))
        #model.train()
        ...
        #optimizer = torch.optim.Adam(params=grad_vars, lr=args.lrate, betas=(0.9, 0.999))
        #optimizer.zero_grad()
        img_loss = img2mse(raw, target)
        #img_loss = img2mse(raw, fake_gt)
        ...
        torch.save({
            'global_step': 199999,
            'network_fn_state_dict': model_fp32,
            'network_fine_state_dict': model_fp32,
            'optimizer_state_dict': optimizer.state_dict(),
        }, path)
        print('Saved checkpoints at', path)
    '''
