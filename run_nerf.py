...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    for k in all_ret:
        ...
        #print("k_sh",k_sh)
        #print("result size",all_ret[k].shape)
        all_ret[k] = torch.reshape(all_ret[k], k_sh)
    #print('rgb size',all_ret['rgb_map'].shape)
...
def create_nerf(args):
    ...
    if args.ft_path is not None and args.ft_path!='None':
        ...
        print(os.path.join(basedir, expname))
...
def config_parser():
    ...
    parser.add_argument("--convert", type=bool, default=False, 
                        help='convert the model to quantized model and do BFA')

    # training options
    parser.add_argument("--rounds", type=int, default=1, 
                        help='bfa rounds in charge of flipped bits')
...
def train():
    ...
    if args.convert == True:
        ...
        for m in imodel.modules():
            if isinstance(m, quan_Linear):
                m.__reset_stepsize__()
                m.__reset_weight__()
        
        weight_conversion(imodel)
        ...
        raw = network_q_fn(ori,rays_d,model)
        import copy
        
        imodel_clone = copy.deepcopy(imodel)    
        #psnr=40
        for j in range(avgs):
            imodel=imodel.cuda()
            data_p = ori
            ...
            #while psnr >= 10:
            for r in range(rounds):
                layer[j,r+1],offsets[j,r+1]=attacker1.progressive_bit_search(imodel,network_q_fn, data_p,data_d,data, target,data1,target1.long(),render_kwargs_train)
                print(r+1)
                #_,acc[j,r+1]=validate(model, network_q_fn,device, criterion, data_p,data_d,target, 0)
                #_,temp[j,r+1]=validate2(model, network_q_fn,device, criterion, data_p,data_d,target, 0)
        ...
        #print(psnr_list)
        i=0
        ## function check how many bits are flipped
        for name, m in imodel_clone.named_modules():
            #print(type(m))
            if isinstance(m, quan_Linear):
                i=i+1
                j=0
                for name1, h in imodel.named_modules():
                    #print(type(h))
                    if isinstance(h, quan_Linear):
                        j=j+1
                        if i==j:
