...
def train():
    ...
    maxgradid = []
    layers = []
    opgrad = optimizer.state_dict()['state']
    print(len(opgrad))
    for i in range(len(opgrad)):
                w_v2,w_id2 = opgrad[i]['exp_avg'].detach().abs().view(-1).topk(1)
                t = (opgrad[i]['exp_avg'] == w_v2).nonzero(as_tuple=False)
                if t.nelement() != 0 :
                    tp = t
                else:
                    tp = (opgrad[i]['exp_avg'] == -w_v2).nonzero(as_tuple=False)
    ...
    '''
    for name, idxs, grads in zip(layers, maxgradid, maxgrad):
